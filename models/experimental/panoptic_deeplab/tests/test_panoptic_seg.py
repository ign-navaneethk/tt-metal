...
from models.experimental.panoptic_deeplab.reference.panoptic_seg import PanopticDeepLabUnified as TorchPanopticDeepLab
from models.experimental.panoptic_deeplab.tt.panoptic_seg import TTPanopticDeepLabUnified
...
class PanopticDeepLabUnifiedTestInfra:
    def __init__(
        ...
        # logger.info("Torch model: ", torch_model)
        ...
        dummy_x = torch.randn(1, 2048, 32, 64)
        dummy_res3 = torch.randn(1, 512, 64, 128)
        dummy_res2 = torch.randn(1, 256, 128, 256)

        # For semantic decoder
        if hasattr(parameters, "semantic_decoder"):
            # ASPP
            aspp_args = infer_ttnn_module_args(
                model=torch_model.semantic_decoder.aspp, run_model=lambda model: model(dummy_x), device=None
            )
            if hasattr(parameters.semantic_decoder, "aspp"):
                parameters.semantic_decoder.aspp.conv_args = aspp_args

            # Res3
            aspp_out = torch_model.semantic_decoder.aspp(dummy_x)
            ...
            # Res2
            res3_out = torch_model.semantic_decoder.res3(aspp_out, dummy_res3)
        ...
        # print("PyTorch model output: ", self.torch_output_tensor)
        # logger.info("PyTorch model output: ", self.torch_output_tensor)
    ...
    def validate(self, output_tensor=None):
        ...
        logger.info(
            f"Panoptic DeepLab Unified - Semantic Logits: batch_size={self.batch_size}, "
...
@pytest.mark.parametrize(
    ...
    [
        (1, 3, 512, 1024),  # Standard test size
        # (1, 3, 1024, 2048),  # Full cityscapes resolution (uncomment for larger test)
    ],
)
