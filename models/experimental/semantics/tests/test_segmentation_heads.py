...
from models.experimental.deeplab_segmentation.tt.tt_panoptic_deeplab_segmentation import PanopticDeeplabSemanticsSegmentation
from models.experimental.deeplab_segmentation.reference.panoptic_deeplab_segmentation_head import PanopticDeeplabSemanticsSegmentationModel
...
class PanopticDeeplabSemanticsSegmentationTestInfra:
    def __init__(
        ...
        self.fake_tensor_1 = torch.randn((1,2048,64,128),dtype=torch.float32)
        self.fake_tensor_2 = torch.randn((1,512,128,256),dtype=torch.float32)
        self.fake_tensor_3 = torch.randn((1,256,256,512),dtype=torch.float32)
        ...
        ## ttnn
        tt_host_tensor_1 = ttnn.from_torch(self.fake_tensor_1.permute(0, 2, 3, 1),dtype=ttnn.bfloat16,device=device,mesh_mapper=self.inputs_mesh_mapper,)
        tt_host_tensor_2 = ttnn.from_torch(self.fake_tensor_2.permute(0, 2, 3, 1),dtype=ttnn.bfloat16,device=device,mesh_mapper=self.inputs_mesh_mapper,)
        tt_host_tensor_3 = ttnn.from_torch(self.fake_tensor_3.permute(0, 2, 3, 1),dtype=ttnn.bfloat16,device=device,mesh_mapper=self.inputs_mesh_mapper,)
