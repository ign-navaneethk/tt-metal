...
class PanopticDeeplabSemanticsSegmentationModel(torch.nn.Module):
    ...
    def forward(self, x, res3, res2):
        ...
        t4 = nn.functional.interpolate(t4,(64,128),mode="bilinear")
...
class PanopticDeeplabInstanceSegmentationModel(torch.nn.Module):
    def __init__(self):
        ...
        self.Ins_Seg_Offset_head_conv_depthwise = nn.Sequential(nn.Conv2d(128,128,5,1,2,1,128), nn.ReLU())
    ...
    def forward(self, x, res3, res2):
        ...
        offset_pred = self.Ins_Seg_Offset_head_conv_depthwise(y)
        center_pred = self.Ins_Seg_Center_head_conv_0(y)

        offset_pred = self.Ins_Seg_Offset_head_pointwise(offset_pred)
        center_pred = self.Ins_Seg_Center_head_conv_1(center_pred)

        offset_pred = self.Ins_Seg_offset_predictor(offset_pred)
        center_pred = self.Ins_Seg_center_predictor(center_pred)

        offset_pred = nn.functional.interpolate(offset_pred,scale_factor=4,mode="bilinear")
        offset_pred = torch.mul(offset_pred,4)
        center_pred = nn.functional.interpolate(center_pred,scale_factor=4,mode="bilinear")
